...
| `quantum` | zero, bell |
| `db` | **pool de conexiones + validado e2e** (2026-07-20): SQLite (rusqlite bundled) con **conexión persistente por ruta** (antes reabría el archivo en cada query — coste por llamada y `:memory:` se borraba). Ahora WAL + synchronous=NORMAL + foreign_keys=ON + busy_timeout(5s); `:memory:` **persiste** entre llamadas. API: query/uno/ejecutar, **insertar** (devuelve last_insert_rowid), **transaccion** (lista de `"sql"` o `["sql",[params]]` en una tx), tablas, **cerrar** (saca la conexión del pool). Bench: 5000 inserts en archivo ≈ 390ms en Windows. Pendiente para escala: driver Postgres/MySQL (requiere servidor para validar e2e) |
