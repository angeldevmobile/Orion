    ...
    match function {
        ...
        "containers" => {
            ...
            let list = json.as_array()
                .ok_or("docker.containers: unexpected response")?
                .iter()
        ...
        // docker.start(id) → {ok}
        "start" => {
            if args.is_empty() { return Err("docker.start requires (id)".into()); }
        ...
        "stop" => {
            if args.is_empty() { return Err("docker.stop requires (id)".into()); }
            let id      = to_str(&args[0]);
        ...
        "restart" => {
            if args.is_empty() { return Err("docker.restart requires (id)".into()); }
            let id = to_str(&args[0]);
        ...
        // docker.kill(id) → {ok}
        "kill" => {
            if args.is_empty() { return Err("docker.kill requires (id)".into()); }
            let id = to_str(&args[0]);
        ...
        "remove" => {
            if args.is_empty() { return Err("docker.remove requires (id)".into()); }
            let id    = to_str(&args[0]);
        ...
        "logs" => {
            if args.is_empty() { return Err("docker.logs requires (id)".into()); }
            let id   = to_str(&args[0]);
        ...
        "inspect" => {
            if args.is_empty() { return Err("docker.inspect requires (id)".into()); }
            let id   = to_str(&args[0]);
        ...
        "run" => {
            if args.is_empty() { return Err("docker.run requires (image, opts?)".into()); }
        ...
        "images" => {
            ...
            let list = json.as_array()
                .ok_or("docker.images: unexpected response")?
                .iter()
                .map(image_summary)
                .collect();
            Ok(EvalValue::List(list))
        ...
        "pull" => {
            if args.is_empty() { return Err("docker.pull requires (image)".into()); }
            let image = to_str(&args[0]);
        ...
        "stats" => {
            if args.is_empty() { return Err("docker.stats requires (id)".into()); }
            let id   = to_str(&args[0]);
        ...
        "exec" => {
            if args.len() < 2 {
                return Err("docker.exec requires (id, cmd, opts?)".into());
            }
            let id  = to_str(&args[0]);
            ...
            let exec_id = created["Id"].as_str()
                .ok_or_else(|| "docker.exec: response with no Id".to_string())?
                .to_string();
        ...
        "network_list" => {
            ...
            let list = json.as_array()
                .ok_or("docker.network_list: unexpected response")?
                .iter()
                .map(|v| {
                    let mut m = HashMap::new();
                    m.insert("id".into(),     EvalValue::Str(
        ...
        f => Err(format!(
            "docker.{}() does not exist. Funciones disponibles: config, containers, start, stop, \
