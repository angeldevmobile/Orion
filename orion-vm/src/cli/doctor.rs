...
pub fn run_doctor() {
    ...
    banner::print_banner();
    banner::section("Orion environment diagnostics");

    let mut all_ok = true;

    // 1. Binary version
    banner::row("VM version", &format!("v{}", env!("CARGO_PKG_VERSION")), true);

    // 2. Project + package directories
    let root = paths::project_root();
    banner::row("Project root", &root.to_string_lossy(), true);
    banner::row(
        "Manifiesto",
    ...
    let describe = |d: &std::path::Path| {
        ...
        else { format!("{} (not created yet)", d.display()) }
    };
    banner::row("Project packages", &describe(&paths::project_packages_dir()), true);
    banner::row("Global packages",     &describe(&paths::global_packages_dir()),  true);
    banner::row("Would install in",         &paths::install_dir().to_string_lossy(),   true);

    // 3. Temp write access
    let tmp = std::env::temp_dir().join("orion_doctor_check.tmp");
    let can_write = fs::write(&tmp, b"ok").is_ok();
    let _ = fs::remove_file(&tmp);
    banner::row("Writing to /tmp", if can_write { "OK" } else { "Sin permisos" }, can_write);
    ...
    println!();
    banner::section("Environment variables");
    ...
    if inventario.is_empty() {
        banner::info("No packages installed");
    } else {
    ...
    println!();
    if all_ok {
        banner::ok("All good — Orion is ready to use");
    } else {
        banner::fail("Some checks failed — review the items marked with ✗");
        std::process::exit(1);
    }
}
